...
class ADESegmentation(data.Dataset):
    ...
    def __init__(self,
        ...
        self.data_ratio = opts.data_ratio
        ...
        if image_set=='test':
            ...
            if self.image_set == 'train' and self.curr_step>0:
                file_names = get_dataset_list('ade', self.task, cil_step, image_set, self.overlap)
                num_file_names = len(file_names)
                print('>>>num_all_file for step-{} training:'.format(self.image_set, self.curr_step), num_file_names)
                num_file_names_LD = round(self.data_ratio * num_file_names)
                file_names = file_names[:num_file_names_LD]
                print('>>>num_LD_file for step-{} training:'.format(self.image_set, self.curr_step), num_file_names_LD)
            else:
                file_names = get_dataset_list('ade', self.task, cil_step, image_set, self.overlap)
                print('>>>num_file for {}:'.format(self.image_set), len(file_names))
