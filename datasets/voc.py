    ...
    def __init__(self,
        ...
        if image_set=='test':
            ...
            if self.image_set == 'train' and self.curr_step>0:
                ...
                print('>>>num_all_file for step-{} {}:'.format(self.curr_step, self.image_set), num_file_names)
                num_file_names_LD = round(self.data_ratio * num_file_names)
                file_names = file_names[:num_file_names_LD]
                print('>>>num_LD_file for step-{} {}:'.format(self.curr_step, self.image_set), num_file_names_LD)
            else:
                file_names = get_dataset_list('voc', self.task, cil_step, image_set, self.overlap)
                print('>>>{} files for {}:'.format(len(file_names), self.image_set))
