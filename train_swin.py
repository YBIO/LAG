...
def get_argparser():
    ...
    parser.add_argument("--vis_port", type=str, default='28333',  help='port for visdom')
...
def main(opts):
    ...
    if opts.overlap:
        ckpt_str = "/hdd0/yb/code/CISSLD/checkpoints/%s_%s_%s_step_%d_overlap.pth"
    else:
        ckpt_str = "/hdd0/yb/code/CISSLD/checkpoints/%s_%s_%s_step_%d_disjoint.pth"
    ...
    while cur_itrs < total_itrs:
        ...
        with torch.cuda.amp.autocast(enabled=opts.amp):
            ...
            print('ret_features, outputs', ret_features.size(), outputs.size())
            # feature decoupling
            if opts.feature_decoupling:
            ...
            if model_prev is not None and opts.curr_step>0:
                with torch.no_grad():
                    ...
                    print('ret_features_prev, outputs_prev', ret_features_prev.size(), outputs_prev.size())
                    assert False
                ...
                if opts.feature_decoupling:
                    ...
                    KD_loss_ret = KD_loss_ret_out + DEC_loss
                else:
                    KD_loss_ret = KD_loss_ret_out 
            ...
            if opts.pseudo and opts.curr_step > 0:
                ...
                if opts.uncertainty_pseudo:
                    ...
                    print('ranking_map:', ranking_map.size(), ranking_map[11][200,200], torch.nonzero(ranking_map))
 
                    pseudo_labels = torch.where( (labels <= fg_idx) & (pred_labels > fg_idx) & (pred_scores >= opts.pseudo_thresh) & (ranking_map > opts.pseudo_thresh), 
                                            pred_labels, 
                                            labels)
                else: 
